...
import type { ChainId, Offer } from '../lib/types'
...
function cheapest(fams: string[]): Offer | null {
...
const ranked = computed<RankedRecipe[]>(() => {
  const list: RankedRecipe[] = recipes.map((recipe) => {
    ...
    const r: RankedRecipe = { recipe, matches, onSpecial, total: required.length, chains, estCost: 0, perServe: Infinity, rank: 0 }
